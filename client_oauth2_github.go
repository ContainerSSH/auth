...
type GitHubConfig struct {
	...
	// GitHub server certificate. This must be set on Windows.
	...
	// org:read scope to be granted by the user. If the user does not grant this scope the authentication fails.
	RequireOrgMembership string `json:"requireOrgMembership" yaml:"requireOrgMembership"`
	// Require2FA requires the user to have two factor authentication enabled when logging in to this server.
...
func (p *gitHubProvider) createFlow(connectionID string, username string) (
	...
	apiClient, err := http.NewClient(p.apiClientConfig, logger)
	if err != nil {
		return gitHubFlow{}, log.WrapUser(
			err,
			EGitHubHTTPClientCreateFailed,
...
func (p *gitHubProvider) getScope() string {
	...
	if p.requiredOrgMembership != "" {
		...
		for _, scope := range scopes {
			if scope == "org" || scope == "org:read" {
				foundOrgRead = true
				break
			}
		}
		if !foundOrgRead {
			scopes = append(scopes, "org:read")
		}
	}
...
func (g *gitHubFlow) checkGrantedScopes(scope string) error {
	...
	if g.provider.requiredOrgMembership != "" {
		for _, grantedScope := range grantedScopes {
			if grantedScope == "org" || grantedScope == "org:read" {
				return nil
			}
		}
		err := log.UserMessage(
			...
			"The user has not granted the org or org:read memberships required to validate the organization member ship.",
		)
		g.logger.Debug(err)
		return err
	}
...
loop:
	for {
		...
		statusCode, lastError = g.apiClient.Get("/user", response)
		if lastError == nil {
			if statusCode == 200 {
...
loop:
	for {
		...
		resp := &gitHubDeleteAccessTokenResponse{}
		statusCode, err := g.apiClient.Delete(
...
loop:
	for {
		...
		statusCode, lastError = g.jsonClient.Post("/login/oauth/access_token", req, resp)
		if statusCode != 200 {
