...
func (o *oauth2Client) Password(_ string, _ []byte, _ string, _ net.IP) AuthenticationContext {
	return &oauth2Context{false, nil, log.UserMessage(
		EUnsupported,
...
func (o *oauth2Client) KeyboardInteractive(
			...
			if err == nil {
				...
				if err != nil {
					...
					if code, ok := answers.Answers["code"]; ok {
						...
						if len(parts) != 2 {
							return &oauth2Context{false, metadata, log.UserMessage(
								EAuthFailed,
	...
	return &oauth2Context{false, metadata, log.WrapUser(err, EAuthFailed, "Authentication failed.", "Authentication failed."), nil}
}
